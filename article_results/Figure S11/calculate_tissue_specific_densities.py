...
def get_ts_info(Dataset):
    ...
    for rowNum, row in Dataset.iterrows():
        ...
        if chromosome not in chromosome_ts_info:
            chromosome_ts_info[chromosome] = {}
            chromosomal_total_genes[chromosome] = 0
        chromosomal_total_genes[chromosome] += 1
        
        for item in row:
            tissue, feature = split_name(item)
            if 'expression' == feature:
                
                if tissue not in chromosome_ts_info[chromosome]:
                    chromosome_ts_info[chromosome][tissue] = 0
                if row[item] == row[item] and row[item]!='':
                    if row[item] > 2:
                        chromosome_ts_info[chromosome][tissue] += 1
...
def calculate_ts_density(chromrosome_ts_info_median, chromosomal_total_genes):
    ts_density_per_arm_tissue = {'Chromosome':[]}
    tissues = list(chromrosome_ts_info_median[1].keys())
    for t in tissues:
        ts_density_per_arm_tissue[t]=[]

    for chromosome in chromrosome_ts_info_median:
        ts_density_per_arm_tissue['Chromosome'].append(chromosome)
    ...
    ts_density_per_arm_tissue_dataframe.to_csv('processed_datasets/Tissue_specific_gene_densities.csv', index = False)
    return ts_density_per_arm_tissue_dataframe

def add_feature_to_dataset(Dataset, ts_gene_densities):
    '''
    ...
    '''
    ts_density_dict = {}
    cancer_tissue_dict = get_cancers_associated_tissue('TCGA','GTEx')                    #This function from get_info file
    density_feature = []
    for rowNum, row in ts_gene_densities.iterrows():
        row = row.to_dict()
        ts_density_dict[row['Chromosome']] = row
...
def ts_densities_main(Dataset_genes,arm_cancer_dataset):
    ...
    Dataset = add_feature_to_dataset(arm_cancer_dataset, ts_gene_densities)
    return Dataset
