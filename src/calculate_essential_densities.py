...
def add_feature_to_dataset(Dataset, essential_density, data_type):
    '''
    ...
    cancer_tissue_dict = get_cancers_associated_tissue(data_type, 'DepMap')  # This function from get_info file
    density_feature = []
...
def crispr_densities_main(Dataset_genes, arm_cancer_dataset, data_type):
    if data_type == 'TCGA':
        ...
        essential_density = pd.read_csv('Processed datasets/essential_gene_density.csv')
    arm_cancer_dataset = add_feature_to_dataset(arm_cancer_dataset, essential_density, data_type)
