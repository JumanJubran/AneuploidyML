...
def get_cancers_associated_tissue(data_type,feature_type):
    ...
    if data_type == 'CCL':
        cancer_tissue = pd.read_csv("../Datasets/CCL_cancer_tissue_association")
    cancer_tissue_dict = {}
...
def get_aneuploidy_label():
    data = pd.read_csv("/gpfs0/estiyl/users/juman/Projects/Aneuploidy most important/GISTIC2/Summerized tabel 2.csv")
    aneuploidy_labels = {}
    for rowNum, row in data.iterrows():
        row = row.to_dict()
        for item in row:
