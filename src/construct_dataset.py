...
def get_features_values(row,g_tissue,e_tissue,c_type, data_type):
    features_values = {}
    for item in row:
        if item not in ['chromosome arm']:
            item2 = item.split('_')
...
def calculation_per_ChromArm(z_score_dataset, disease_tissue_association, arm_cancer_dataset,data_type):
    ...
    for rowNum,row in z_score_dataset.iterrows():
        ...
        if row['chromosome arm'] != '21p':
            ...
            for rowNum2, row2 in disease_tissue_association.iterrows():
                ...
                features_values = get_features_values(row,g_tissue,e_tissue,row2['Type'].lower(),data_type)
...
def add_labels(Dataset, datatype):
    if datatype == 'TCGA':
        path = '../article_results/Figure 1/'
    else:
        path = '../article_results/Figure 3/'

    arm_calls = pd.read_csv(path + "panel A _ cancer_arm_aneuploidy.csv")
...
def construct_Dataset_main(Dataset_arm, Dataset_genes_zscore, Dataset_genes, data_type):
    ...
    if data_type == 'TCGA':
        ...
        disease_tissue_association = pd.read_csv("../Datasets/CCL data/cancer_tissue_association.csv")

    features_types = get_features_types(Dataset_arm)
    arm_cancer_dataset = assign_fieldnames(features_types)


    arm_cancer_dataset = calculation_per_ChromArm(Dataset_arm, disease_tissue_association, arm_cancer_dataset, data_type)  # construct dataset as pairs of cancer type and chromosome arm
    ...
    if data_type == 'TCGA':
        arm_cancer_dataset = ts_densities_main(Dataset_genes_zscore,arm_cancer_dataset, data_type)         # add tissue-specific gene density feature
    elif data_type == 'CCL':
        arm_cancer_dataset = ts_densities_main('',  arm_cancer_dataset, data_type)  # add tissue-specific gene density feature

    print('---------- Calculate essential gene density based on CRISPR and add it to dataset ----------')
    if data_type == 'TCGA':
        arm_cancer_dataset = crispr_densities_main(Dataset_genes, arm_cancer_dataset,data_type)           # add essential gene density feature
    else:
        arm_cancer_dataset = crispr_densities_main('', arm_cancer_dataset,data_type)  # add essential gene density feature
    ...
    arm_cancer_dataset = standardization(arm_cancer_dataset,data_type)
    arm_cancer_dataset['paralogs ratio highest identity'] = arm_cancer_dataset['paralogs ratio highest identity']*-1
    arm_cancer_dataset = add_labels(arm_cancer_dataset, data_type)
