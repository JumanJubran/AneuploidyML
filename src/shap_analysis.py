...
def SHAP_summary_plot(shap_values, X_test,file_name):
    ...
    if 'Rest' in file_name:
        fig.savefig('../article_results/Figure S5/'+file_name+'__SHAP_direction.png',bbox_inches='tight')
    else:
...
def SHAP_bar_plot(shap_values, X_test, file_name):
    ...
    if 'Rest' in file_name:
        fig.savefig('../article_results/Figure S5/'+file_name+'__SHAP_top10_features_contribution.png',bbox_inches='tight')
    else:
...
def save_features_importance_as_output_file(X, shap_values, file_name):
    ...
    if 'Rest' in file_name:
        pd.DataFrame.from_dict(dict_to_write).to_csv('../article_results/Figure S5/'+file_name+'__feature_importance_values.csv', index=False)
    else:
...
def process_dataset_per_model(data,c1,c2):
    ...
    print(labels, class_to_drop, threshold, total_count)
    if c2 == 'Neutral':
